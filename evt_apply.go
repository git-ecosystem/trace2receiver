...
//
// (If this is a Git command, it may independently generate Trace2
...
// started it.  We use that to make the child span a child of the
// top frame on that thread's region stack.  This helps make the
// child appear in flame graphs in proper context.  But the because
// the child may be async, the later child-exit event may or may not
// have the same open region in the thread's region stack, so the
// matchup is approximate.  Note that we're only setting the SpanID
// in the child object -- we're not attaching the child object to
// the thread-object; we keep an array of children for that.
//
// Also, the child itself will inherit the SID of the current process,
// so for Git child processes, the telemetry generated by the child
// will be a direct child span of the currrent process span, rather
// than the region.
func apply__child_start(tr2 *trace2Dataset, evt *TrEvent) (err error) {
	...
	if !ok {
		...
		// Re-assign it to the "main" thread.
		//
		// TODO log debug warning.
		th = &tr2.process.mainThread
	}
...
//
// Use the same thread-parenting rules as we do for child_start.
